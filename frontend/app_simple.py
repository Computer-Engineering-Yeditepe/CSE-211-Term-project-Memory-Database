...
def query():
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if response_data.get('status') == 'success' and 'result' in response_data:
                # Query sonucunu formatla
                result = response_data['result']
                rows = result.get('rows', [])
                columns = result.get('columns', [])
                result_msg = f"Found {len(rows)} row(s)\n\n"
                if rows:
                    # Tablo formatında göster
                    result_msg += " | ".join(columns) + "\n"
                    result_msg += "-" * (len(" | ".join(columns))) + "\n"
                    for row in rows:
                        values = [str(row.get(col, '')) for col in columns]
                        result_msg += " | ".join(values) + "\n"
                else:
                    result_msg += "No rows returned."
            else:
                result_msg = response_data.get('msg', 'Query executed but no result')
